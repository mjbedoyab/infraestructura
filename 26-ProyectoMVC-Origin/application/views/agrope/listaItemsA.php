												...
												<?php foreach ($items as $key => $item) : ?>
													<tr>
														...
														<td class="text-center">
															<a href="<?= base_url('index.php/agrope/Items/createItemsA/' . $item->id_item) ?>" class="btn btn-outline-info">
																<i class="nav-icon fa-solid fa-pen-nib" style="color: #0d58d9;"></i>
															</a>
														</td>
														<td class="text-center">
															<a href="<?= base_url('index.php/agrope/Items/borrar/' . $item->id_item) ?>" 
	...
	function confirmarEliminacion(id) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				window.location.href = "<?= base_url('index.php/agrope/Items/borrar/') ?>" + id;
			}
		});
	}
